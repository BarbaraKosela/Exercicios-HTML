...
namespace Model.Repositório
{
    public class AlunosRepositorio
    {
        ...
        {
            SqlCommand comando = new BancoDados().ObterConexao();
            comando.CommandText = @"INSERT INTO alunos (nome,matricula,nota01,nota02,nota03,frequencia) OUTPUT
            INSERTED.ID VALUES (@NOME, @MATRICULA, @NOTA01, @NOTA02, @NOTA03, @FREQUENCIA)";
        ...
        {
            SqlCommand comando = new BancoDados().ObterConexao();
            comando.CommandText = "UPDATE alunos SET nome = @NOME, matricula = @MATRICULA, nota01 = @NOTA01, nota02 = @NOTA02, nota03 = NOTA03, frequencia = @FREQUENCIA WHERE id = @ID";
            comando.Parameters.AddWithValue("@NOME", aluno.Nome);
            comando.Parameters.AddWithValue("@MATRICULA", aluno.Matricula);
            comando.Parameters.AddWithValue("@NOTA01", aluno.Nota1);
            comando.Parameters.AddWithValue("@NOTA02", aluno.Nota2);
        ...
        public Alunos ObterPeloID()
        {
            Alunos aluno = null;
            SqlCommand comando = new BancoDados().ObterConexao();
            comando.CommandText = "SELECT nome,matricula,nota01,nota02,nota03,frequencia FROM alunos WHERE id = @ID";
            comando.Parameters.AddWithValue("@ID", id);
            DataTable tabela = new DataTable();
            tabela.Load(comando.ExecuteReader());
